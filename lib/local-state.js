// lib/local-state.js — pure logic for all local-first features (saved jobs,
// recently viewed, visit state). No React, no window access at module level:
// every function takes a storage object, so the exact code that runs in the
// browser is unit-tested in Node with a fake storage.
//
// Storage contract (per the retention-sprint brief):
//   - namespaced keys:            earlyaijobs:saved-jobs, :recently-viewed, :visit-state
...
export const KEYS = {
  ...
  recent: "earlyaijobs:recently-viewed",
...
export function readRecent(storage) {
  const data = safeRead(storage, KEYS.recent);
  ...
  for (const entry of data.jobs) {
    ...
    seen.add(id);
    jobs.push({ id, viewedAt: entry.viewedAt || null });
  }
  return { version: 1, jobs: jobs.slice(0, RECENT_MAX) };
}

/** Record a view: newest first, duplicates move to front, capped at 20. */
export function recordView(storage, id, now = new Date().toISOString()) {
  const sid = String(id);
  if (!validId(sid)) return readRecent(storage).jobs;
  const data = readRecent(storage);
  const rest = data.jobs.filter((j) => j.id !== sid);
  const jobs = [{ id: sid, viewedAt: now }, ...rest].slice(0, RECENT_MAX);
  safeWrite(storage, KEYS.recent, { version: 1, jobs });
  return jobs;
}

export function recentIds(storage) {
  return readRecent(storage).jobs.map((j) => j.id);
}
