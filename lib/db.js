...
export async function getJobs({ category, company, remote, country, posted, q, page = 1, perPage = 50 } = {}) {
  let query = applyLang(
    supabase
      .from("jobs")
  ...
  if (posted && POSTED_WINDOWS[posted]) {
    const since = new Date(Date.now() - Number(posted) * 24 * 3600000).toISOString();
    query = query.gte("first_published", since);
  }
...
export async function getJob(id) {
  const { data, error } = await supabase
    .from("jobs")
    .select("id, title, company_name, location, url, category, workplace_type, is_remote, employment_type, description, first_published, first_seen_at, last_seen_at, is_open")
    .eq("id", id)
    .single();
