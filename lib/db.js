...
export const COMPANY_DESCRIPTIONS = {
  ...
  togetherai:
    "Together AI runs a cloud platform for training, fine-tuning, and serving open-source AI models, combining GPU " +
    "infrastructure with inference and research. Headquartered in San Francisco with teams in Amsterdam, Singapore, " +
    "and India, it hires across research, infrastructure engineering, and go-to-market.",
  sierra:
    "Sierra builds conversational AI agents that companies deploy for customer-facing work, founded by Bret Taylor " +
    "and Clay Bavor. Based in San Francisco, it hires across engineering, product, and a fast-growing enterprise " +
    "sales and solutions organization.",
  harvey:
    "Harvey builds AI tools for legal work, used by law firms and corporate legal teams for research, drafting, " +
    "and analysis. Headquartered in San Francisco with a growing international presence, it hires across " +
    "engineering, research, legal domain expertise, and go-to-market.",
};

// Apply-URL allowlist. Job URLs come from third-party feeds we don't control,
// so they are untrusted input: a compromised or poisoned feed must not be able
// to turn an "Apply on OpenAI's site" button into a phishing destination.
// Only these hosts (and their subdomains) can ever be rendered as an Apply
// link; anything else falls back to the company's own careers page.
const APPLY_HOSTS = [
  "greenhouse.io",      // job-boards.greenhouse.io, boards.greenhouse.io
  "ashbyhq.com",        // jobs.ashbyhq.com
  "lever.co",           // jobs.lever.co
  "databricks.com",     // databricks routes its Greenhouse jobs through its own site
];

/** @returns {string|null} the URL if it targets a known ATS/company host */
export function safeApplyUrl(raw) {
  try {
    const u = new URL(String(raw));
    if (u.protocol !== "https:") return null;
    const host = u.hostname.toLowerCase();
    const allowed = APPLY_HOSTS.some((h) => host === h || host.endsWith(`.${h}`));
...
export const COMPANY_LOGOS = {
  ...
  // Official marks, supplied by Patrick 2026-08-23. All are wide wordmark
  // lockups, hence their membership in WIDE_LOGOS below.
  cohere: "/companies/cohere.png",
  perplexity: "/companies/perplexity.png",
  cursor: "/companies/cursor.png",
  cognition: "/companies/cognition.png",
  mistral: "/companies/mistral.png",
  figureai: "/companies/figureai.png",
  coreweave: "/companies/coreweave.png",
  togetherai: "/companies/togetherai.png",
  sierra: "/companies/sierra.png",
  harvey: "/companies/harvey.png",
};
...
export const WIDE_LOGOS = new Set([
  "elevenlabs", "cohere", "perplexity", "cursor", "cognition", "mistral",
  "figureai", "coreweave", "togetherai", "sierra", "harvey",
]);
...
export async function getJob(id) {
  const { data, error } = await supabase
    .from("jobs")
    // location_countries feeds the JobPosting structured data: without it the
    // schema module cannot emit addressCountry or, for remote roles, the
    // applicantLocationRequirements field Google REQUIRES beside TELECOMMUTE.
    // Its absence here was the one critical error in the 2026-08-24 Rich
    // Results validation.
    .select("id, title, company_name, location, url, category, workplace_type, is_remote, location_countries, employment_type, description, description_html, first_published, first_seen_at, last_seen_at, is_open")
    .eq("id", id)
    .single();
...
/** Counts per company WITHIN one category — the "top hiring companies" block
 *  on /jobs/[category] pages. Same head-only count pattern as everything else;
 *  returns [{slug, count}] sorted descending, companies with zero omitted. */
export async function getCategoryCompanyCounts(category) {
  const results = await Promise.all(
    APPROVED_COMPANIES.map((slug) =>
      countMatching((q) => q.eq("company_name", slug).eq("category", category))
    )
  );
  return APPROVED_COMPANIES
    .map((slug, i) => ({ slug, count: results[i] }))
    .filter((c) => c.count > 0)
