...
// ---------------------------------------------------------------------------
// The rendered job page strips paragraphs that are nothing but an ATS
// requisition code ("RDQ427R146"); the structured data must describe the same
// content a visitor sees, so the schema description gets the same treatment.
const { stripReferenceCodes } = require("../description-format");
...
function buildJobPosting(job, opts) {
  ...
  const entity = {
    ...
    description: job.description_html
      ? stripReferenceCodes(job.description_html)
...
function buildBreadcrumbs(job, opts) {
  ...
  if (job.category && categoryLabels[job.category]) {
    items.push({ name: categoryLabels[job.category], item: `${base}/jobs/${job.category}` });
  }
  items.push({
